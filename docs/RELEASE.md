...
- [ ] **The Windows command list in [CONTRIBUTING.md](../CONTRIBUTING.md) run on a Windows machine**, at the tagged commit. There is no runner left that executes a Windows binary (§5), and Windows is one of the six platforms every release ships. The list is what the deleted `windows` job ran, kept as commands rather than as YAML for exactly this.
